...
func (ctx *symmetricKeyCipher) decryptKey(headers rawHeader, recipient *recipientInfo, generator keyGenerator) ([]byte, error) {
	...
	case A128GCMKW, A192GCMKW, A256GCMKW:
		...
		if err != nil {
			return nil, fmt.Errorf("timo972/go-jose: invalid IV: %v", err)
		}
		tag, err := headers.getTag()
		if err != nil {
			return nil, fmt.Errorf("timo972/go-jose: invalid tag: %v", err)
		}
	...
	case PBES2_HS256_A128KW, PBES2_HS384_A192KW, PBES2_HS512_A256KW:
		...
		if err != nil {
			return nil, fmt.Errorf("timo972/go-jose: invalid P2S: %v", err)
		}
		if p2s == nil || len(p2s.data) == 0 {
			return nil, fmt.Errorf("timo972/go-jose: invalid P2S: must be present")
		}

		p2c, err := headers.getP2C()
		if err != nil {
			return nil, fmt.Errorf("timo972/go-jose: invalid P2C: %v", err)
		}
		if p2c <= 0 {
			return nil, fmt.Errorf("timo972/go-jose: invalid P2C: must be a positive integer")
		}
		if p2c > 1000000 {
			// An unauthenticated attacker can set a high P2C value. Set an upper limit to avoid
			// DoS attacks.
			return nil, fmt.Errorf("timo972/go-jose: invalid P2C: too high")
		}
...
func (ctx symmetricMac) verifyPayload(payload []byte, mac []byte, alg SignatureAlgorithm) error {
	...
	if err != nil {
		return errors.New("timo972/go-jose: failed to compute hmac")
	}

	if len(mac) != len(expected) {
		return errors.New("timo972/go-jose: invalid hmac")
	}
