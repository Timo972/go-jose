...
func (k JSONWebKey) MarshalJSON() ([]byte, error) {
	...
	default:
		return nil, fmt.Errorf("timo972/go-jose: unknown key type '%s'", reflect.TypeOf(key))
	}
	...
	if x5tSHA1Len > 0 {
		if x5tSHA1Len != sha1.Size {
			return nil, fmt.Errorf("timo972/go-jose: invalid SHA-1 thumbprint (must be %d bytes, not %d)", sha1.Size, x5tSHA1Len)
		}
		raw.X5tSHA1 = base64.RawURLEncoding.EncodeToString(k.CertificateThumbprintSHA1)
	}
	if x5tSHA256Len > 0 {
		if x5tSHA256Len != sha256.Size {
			return nil, fmt.Errorf("timo972/go-jose: invalid SHA-256 thumbprint (must be %d bytes, not %d)", sha256.Size, x5tSHA256Len)
		}
	...
	if len(k.Certificates) > 0 {
		...
		if len(k.CertificateThumbprintSHA1) > 0 && !bytes.Equal(k.CertificateThumbprintSHA1, expectedSHA1[:]) {
			return nil, errors.New("timo972/go-jose: invalid SHA-1 thumbprint, does not match cert chain")
		}
		if len(k.CertificateThumbprintSHA256) > 0 && !bytes.Equal(k.CertificateThumbprintSHA256, expectedSHA256[:]) {
			return nil, errors.New("timo972/go-jose: invalid or SHA-256 thumbprint, does not match cert chain")
		}
	}
...
func (k *JSONWebKey) UnmarshalJSON(data []byte) (err error) {
	...
	if err != nil {
		return fmt.Errorf("timo972/go-jose: failed to unmarshal x5c field: %s", err)
	}
	...
	case "oct":
		if certPub != nil {
			return errors.New("timo972/go-jose: invalid JWK, found 'oct' (symmetric) key with cert chain")
		}
	...
	case "OKP":
		if raw.Crv == "Ed25519" && raw.X != nil {
			...
			return fmt.Errorf("timo972/go-jose: unknown curve '%s'", raw.Crv)
		}
	default:
		return fmt.Errorf("timo972/go-jose: unknown json web key type '%s'", raw.Kty)
	}
	...
	if certPub != nil && keyPub != nil {
		if !reflect.DeepEqual(certPub, keyPub) {
			return errors.New("timo972/go-jose: invalid JWK, public keys in key and x5c fields do not match")
		}
	}
	...
	*k = JSONWebKey{Key: key, KeyID: raw.Kid, Algorithm: raw.Alg, Use: raw.Use, Certificates: certs}
	...
	if raw.X5u != "" {
		...
		if err != nil {
			return fmt.Errorf("timo972/go-jose: invalid JWK, x5u header is invalid URL: %w", err)
		}
	}
	...
	if err != nil {
		return errors.New("timo972/go-jose: invalid JWK, x5t header has invalid encoding")
	}
	...
	if len(x5tSHA1bytes) == 2*sha1.Size {
		...
		if err != nil {
			return fmt.Errorf("timo972/go-jose: invalid JWK, unable to hex decode x5t: %v", err)
	...
	if err != nil {
		return errors.New("timo972/go-jose: invalid JWK, x5t#S256 header has invalid encoding")
	}

	if len(x5tSHA256bytes) == 2*sha256.Size {
		hx256, err := hex.DecodeString(string(x5tSHA256bytes))
		if err != nil {
			return fmt.Errorf("timo972/go-jose: invalid JWK, unable to hex decode x5t#S256: %v", err)
		}
	...
	if x5tSHA1Len > 0 && x5tSHA1Len != sha1.Size {
		return errors.New("timo972/go-jose: invalid JWK, x5t header is of incorrect size")
	}
	if x5tSHA256Len > 0 && x5tSHA256Len != sha256.Size {
		return errors.New("timo972/go-jose: invalid JWK, x5t#S256 header is of incorrect size")
	}
	...
	if len(k.Certificates) > 0 {
		...
		if len(k.CertificateThumbprintSHA1) > 0 && !bytes.Equal(sha1sum[:], k.CertificateThumbprintSHA1) {
			return errors.New("timo972/go-jose: invalid JWK, x5c thumbprint does not match x5t value")
		}

		if len(k.CertificateThumbprintSHA256) > 0 && !bytes.Equal(sha256sum[:], k.CertificateThumbprintSHA256) {
			return errors.New("timo972/go-jose: invalid JWK, x5c thumbprint does not match x5t#S256 value")
		}
	}
...
func ecThumbprintInput(curve elliptic.Curve, x, y *big.Int) (string, error) {
	...
	if len(x.Bytes()) > coordLength || len(y.Bytes()) > coordLength {
		return "", errors.New("timo972/go-jose: invalid elliptic key (too large)")
	}
...
func (key rawJSONWebKey) rsaPublicKey() (*rsa.PublicKey, error) {
	if key.N == nil || key.E == nil {
		return nil, fmt.Errorf("timo972/go-jose: invalid RSA key, missing n/e values")
	}
...
func (key rawJSONWebKey) ecPublicKey() (*ecdsa.PublicKey, error) {
	...
	case "P-521":
		curve = elliptic.P521()
	default:
		return nil, fmt.Errorf("timo972/go-jose: unsupported elliptic curve '%s'", key.Crv)
	}

	if key.X == nil || key.Y == nil {
		return nil, errors.New("timo972/go-jose: invalid EC key, missing x/y values")
	}
	...
	if curveSize(curve) != len(key.X.data) {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC public key, wrong length for x")
	}

	if curveSize(curve) != len(key.Y.data) {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC public key, wrong length for y")
	}
	...
	if !curve.IsOnCurve(x, y) {
		return nil, errors.New("timo972/go-jose: invalid EC key, X/Y are not on declared curve")
	}
...
func fromEcPublicKey(pub *ecdsa.PublicKey) (*rawJSONWebKey, error) {
	if pub == nil || pub.X == nil || pub.Y == nil {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC key (nil, or X/Y missing)")
	}
	...
	if len(xBytes) > size || len(yBytes) > size {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC key (X/Y too large)")
	}
...
func (key rawJSONWebKey) edPrivateKey() (ed25519.PrivateKey, error) {
	...
	if len(missing) > 0 {
		return nil, fmt.Errorf("timo972/go-jose: invalid Ed25519 private key, missing %s value(s)", strings.Join(missing, ", "))
	}
...
func (key rawJSONWebKey) edPublicKey() (ed25519.PublicKey, error) {
	if key.X == nil {
		return nil, fmt.Errorf("timo972/go-jose: invalid Ed key, missing x value")
	}
...
func (key rawJSONWebKey) rsaPrivateKey() (*rsa.PrivateKey, error) {
	...
	if len(missing) > 0 {
		return nil, fmt.Errorf("timo972/go-jose: invalid RSA private key, missing %s value(s)", strings.Join(missing, ", "))
	}
...
func (key rawJSONWebKey) ecPrivateKey() (*ecdsa.PrivateKey, error) {
	...
	if key.X == nil || key.Y == nil || key.D == nil {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC private key, missing x/y/d values")
	}
	...
	// https://tools.ietf.org/html/rfc7518#section-6.2.1.2
	if curveSize(curve) != len(key.X.data) {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC private key, wrong length for x")
	}

	if curveSize(curve) != len(key.Y.data) {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC private key, wrong length for y")
	}

	// https://tools.ietf.org/html/rfc7518#section-6.2.2.1
	if dSize(curve) != len(key.D.data) {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC private key, wrong length for d")
	}
...
func fromEcPrivateKey(ec *ecdsa.PrivateKey) (*rawJSONWebKey, error) {
	...
	if ec.D == nil {
		return nil, fmt.Errorf("timo972/go-jose: invalid EC private key")
	}
...
func (key rawJSONWebKey) symmetricKey() ([]byte, error) {
	if key.K == nil {
		return nil, fmt.Errorf("timo972/go-jose: invalid OCT (symmetric) key, missing k value")
	}
...
var (
	...
	ErrJWKSKidNotFound = errors.New("timo972/go-jose: JWK with matching kid not found in JWK Set")
)
